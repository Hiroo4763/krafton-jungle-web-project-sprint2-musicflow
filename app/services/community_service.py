...
def create_post(username, title, song_info, content):
    try:
        ...
        return jsonify({"success": True, "message": "게시글이 성공적으로 등록되었습니다."}), 201
    except Exception as e:
        return jsonify({"success": False, "message": "게시글 등록 중 오류 발생", "error": str(e)}), 500
...
def get_posts_paginated(page):
    try:
        ...
        projection = {
            "_id": 1, "nickname": 1, "song_info": 1,
            "likes": 1, "created_at": 1, "content": 1,
            "title": 1, "comment_count": 1,
        }
        ...
        return jsonify({"success": False, "message": "게시글 목록 조회 오류", "error": str(e)}), 500
...
def get_posts_by_similar_taste(current_username, page):
    try:
        ...
        current_user_tastes = set(current_user.get("genres", [])) | set(current_user.get("artists", []))
        ...
        for author_name in all_authors:
            ...
            author_tastes = set(author_user.get("genres", [])) | set(author_user.get("artists", []))
            if len(current_user_tastes.intersection(author_tastes)) >= 3:
        ...
        return jsonify({"success": False, "message": "비슷한 취향 게시글 조회 오류", "error": str(e)}), 500
...
def get_post_by_id(post_id):
    try:
        ...
        return jsonify({"success": False, "message": "게시글 상세 조회 오류", "error": str(e)}), 500
...
def call_post(post_id):
    try:
        ...
        return jsonify({"success": False, "message": "유효하지 않은 게시글 ID"}), 400
...
def increment_like(post_id, user_id):
    try:
        ...
        if user_id in liked_users:
            return False, {"success": False, "message": "이미 좋아요를 누름"}, 400
...
def insert_comment(username, post_id, comment_text):
    ...
    try:
        ...
        return False, {"success": False, "message": "유효하지 않은 게시글 ID"}, 400
    ...
    return True, {"success": True, "message": "댓글 등록 완료", "comment": comment_data}, 200
