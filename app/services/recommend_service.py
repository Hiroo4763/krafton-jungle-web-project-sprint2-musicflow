...
# 깜짝 추천 트랙 1곡 가져오기
async def get_surprise_track(session, all_genres_data, excluded_genre_ids, used_track_ids, user_id=None):
    ...
    if user_id:
        ...
        if user:
            ...
            genre_map = {g["name"].lower(): g["id"] for g in all_genres_data.get("data", [])}
            preferred_genre_ids = [
            ...
            artist_search_results = await asyncio.gather(
                *[get_data(session, f"search/artist?q={name}") for name in preferred_artists]
            )
            preferred_artist_ids = [
                res["data"][0]["id"]
    ...
    candidate_genres = [
        g for g in all_genres_data.get("data", [])
...
async def get_recommendations(user_id=None):
    if user_id:
        ...
        preferred_artists_names, preferred_genres_names = [], []
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if preferred_artist_ids:
            sample_artist_ids = random.sample(preferred_artist_ids, min(len(preferred_artist_ids), 2))
            related_tasks = [get_data(session, f"artist/{aid}/related") for aid in sample_artist_ids]
            for res in await asyncio.gather(*related_tasks):
                if res and res.get("data"):
                    related_artist_ids.update(artist["id"] for artist in res["data"])

        # 후보곡 풀 병렬 수집
        artist_tasks = [fetch_track_pool(session, f"artist/{aid}/top") for aid in preferred_artist_ids]
        related_artist_tasks = [fetch_track_pool(session, f"artist/{aid}/top") for aid in related_artist_ids]
        genre_tasks = [fetch_track_pool(session, f"genre/{gid}/radio") for gid in preferred_genre_ids]

        all_pools = await asyncio.gather(*artist_tasks, *related_artist_tasks, *genre_tasks)
        full_pool = [track for pool in all_pools for track in pool]

        # 중복 제거 후 셔플
        unique_tracks = {track["track_id"]: track for track in full_pool if track}.values()
        shuffled_pool = list(unique_tracks)
        random.shuffle(shuffled_pool)

        main_tracks, used_artists = [], set()
        ...
        if len(main_tracks) < 4:
            used_track_ids = {t["track_id"] for t in main_tracks}
            for track in shuffled_pool:
                if len(main_tracks) >= 4:
                    break
